...
import { X, Mail, ArrowRight, CheckCircle2, ShieldCheck } from "lucide-react";
...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
	...
	const handleSendLink = async (e: React.FormEvent) => {
		...
		const actionCodeSettings = {
			url: window.location.origin, // Automatically uses localhost or your deployed URL
		...
		<div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-sm animate-in fade-in duration-200">
			<div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl relative">
				...
				<div className="mb-8">
					...
					<p className="text-neutral-500 text-sm leading-relaxed">Secure your progress. We'll send a magic link to your inbox for a passwordless login.</p>
				</div>
				...
				{status === "sent" ? (
					<div className="text-center py-6 animate-in zoom-in-95">
						...
						<p className="text-neutral-600 text-sm">A link has been sent to {email}.</p>
					</div>
				) : status === "success" ? (
					<div className="text-center py-6 animate-in zoom-in-95">
					...
					<div className="space-y-6">
						<form onSubmit={handleSendLink} className="space-y-4">
							...
							/>
							{status === "error" && <p className="text-red-500 text-xs font-medium px-1">{errorMessage}</p>}
							<button
								type="submit"
						...
						<button
							onClick={onClose}
							className="w-full py-3 flex items-center justify-center gap-2 text-neutral-500 text-xs font-bold uppercase tracking-widest hover:text-neutral-800 transition-colors"
						>
							<ShieldCheck size={14} /> Continue as Guest
						</button>
					</div>
				)}
			</div>
