			...
			<div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
				{filters.map((f) => (
					<button
						...
						className={`px-4 py-2 rounded-full text-sm font-medium transition-all shrink-0 border ${activeFilter === f.id
					...
					>
						...
						<div className="pt-2 flex justify-between items-center border-t border-neutral-100">
							...
							<button
								...
								className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all border ${post.hasSupported
							...
							<div className="bg-white border border-neutral-200 rounded-3xl p-5 shadow-sm space-y-4">
								...
								<div className="pt-3 flex justify-between items-center border-t border-neutral-100">
									...
									<button
										...
										className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all border ${viewingPost.hasSupported
												? "bg-app-pink/20 border-app-pink text-pink-900"
												: "bg-neutral-50 border-neutral-200 text-neutral-500 hover:bg-neutral-100"
											}`}
									>
							...
							<div className="relative">
								...
								<div className="relative">
									<input
										...
										className={`w-full bg-white border focus:bg-neutral-50 py-3 pl-5 pr-12 text-sm outline-none transition-all placeholder:text-neutral-400 shadow-sm ${replyingTo ? "border-neutral-200 rounded-b-2xl rounded-t-none" : "border-neutral-200 focus:border-neutral-400 rounded-full"
											}`}
										onKeyDown={(e) => {
									...
									<button
										...
										className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full transition-all ${newCommentText.trim() ? "bg-neutral-900 text-white shadow-md hover:scale-105" : "bg-transparent text-neutral-400"
											}`}
									>
					...
					<div className="max-w-md mx-auto p-4 pt-12 space-y-8">
						...
						<div className="space-y-6">
								...
								<div className="flex gap-2">
									{(["win", "struggle", "tip"] as const).map((tag) => (
										<button
											...
											className={`flex-1 py-3 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all border ${newPostTag === tag
							...
							<button
								...
								className={`w-full py-4 rounded-2xl font-medium flex items-center justify-center gap-2 transition-all ${newPostText.trim()
